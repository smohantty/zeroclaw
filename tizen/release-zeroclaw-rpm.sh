...
usage() {
    cat <<'EOF'
usage: ./tizen/release-zeroclaw-rpm.sh
...
The release remote is always `origin`, so if `origin` points at a fork the tag
and GitHub release go to that fork.
EOF
}
...
github_repo_from_url() {
    ...
    if [[ ! "$path" =~ ^[^/]+/[^/]+$ ]]; then
        echo "error: unsupported GitHub remote path: $remote_url" >&2
        exit 1
    fi
...
GH_REPO="$(github_repo_from_url "$(git remote get-url "$REMOTE")")"

echo "Using release remote $REMOTE"
